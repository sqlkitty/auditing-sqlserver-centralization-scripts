/*create this job on any audited server that's sql server version 2017 or later 
...
/****** Object:  Job [Audit Changes Collection]    Script Date: 8/19/2019 11:34:41 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 8/19/2019 11:34:41 AM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [audit sql server changes]    Script Date: 8/19/2019 11:34:42 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'audit sql server changes', 
		...
		@command=N'INSERT INTO centralservername.Auditing.dbo.AuditChanges 
SELECT DATEADD(mi, DATEPART(TZ, SYSDATETIMEOFFSET()), event_time) as event_time, statement, server_instance_name, database_name, schema_name, 
